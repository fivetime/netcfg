...
// setupVPP 处理 VPP 设备（V0：连接 + 兼容性自检 + 列出；不下发流量）。
func setupVPP(global *config.VPPGlobal, v *vppSet) error {
	...
	for _, name := range v.names() {
		slog.Info("VPP-managed device recognized (apply deferred to V1a)", "device", name)
	}
	slog.Info("VPP connection + binding compatibility OK", "devices", len(v.names()))
	return nil
}
